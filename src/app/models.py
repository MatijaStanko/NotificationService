...
class NotificationTypes(SQLModel, table=True):
...
class NotificationTemplate(SQLModel, table=True):
    ...
    channel_id : int = Field(foreign_key="channel_config.id", index=True)
...
class NotificationRequests(SQLModel, table=True):
    __tablename__ = "notification_requests"

    id: Optional[int] = Field(default=None, primary_key=True)
    source_service: Optional[str] = None
    notification_type_id: int = Field(foreign_key="notification_types.id")
    template_id: int = Field(foreign_key="notification_templates.id")
